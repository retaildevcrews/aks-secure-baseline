...
1. Get the cluster name.
   ...
   ```bash
   AKS_CLUSTER_NAME=$(az deployment group show -g rg-bu0001a0008-$TEAM_NAME -n cluster-stamp --query properties.outputs.aksClusterName.value -o tsv)
   ```
...
1. Get AKS `kubectl` credentials.
   ...
   ```bash
   az aks get-credentials -g rg-bu0001a0008-$TEAM_NAME -n $AKS_CLUSTER_NAME
   ```
